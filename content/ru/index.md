...
**unasanu** (= **un**iverse **as** **a** **nu**mber, унасану, вселенная как число) — философская концепция, согласно которой существует любая симулируемая вселенная, потому что её можно представить уже существующим натуральным числом. Из этой концепции можно вывести объяснения для следующих вопросов:
...
Эта идея и её следствия логически выводятся из современного понимания математики, физики и сознания. Но при этом unasanu не требует многомировой интерпретации квантовой механики; бесконечности нашей вселенной или гипотезы об Омега-точке, она опирается на другие, более надёжные и простые основания.
...
* эти идеи заново не переизобретали, 
* сформировать направление дальнейших исследований.

# Виды симуляций

Мы знакомы с идеей симуляции внешнего физического мира на некотором компьютере, эта тема активно обсуждается в поп-культуре и философии. Для начала нужно договориться о том как мы называем разные категории симуляций. Мне кажется что при обсуждении этой темы возникает множество недопониманий, потому что люди редко разделяют эти две категоии.

## Иллюзорная

{{ admonition_start(color="blue", title="📝 Вводимое определение") }}
**Иллюзорная симуляция** — такая симуляция, которая создаёт иллюзию реальности окружающего мира, путём обмана наблюдателя. Другое название таких симуляций — виртуальная реальность [\[6\]]. Пример таких симуляций: фильм «Матрица» [\[3\]], игра «The Sims» [\[4\]] (мир иллюзорен для симов), фильм «13-й этаж» [\[5\]].
{{ admonition_end() }}
...
Людям не нравится идея жить в иллюзорной симуляции. Всем хочется считать что их реальность реальна, и ничто их намеренно не обманывает и не упрощает. Не зря вся поп-культура о жизни в иллюзорной реальности строится на том чтобы выбраться из неё.

## Физическая

{{ admonition_start(color="blue", title="📝 Вводимое определение") }}
**Физическая симуляция** — такая симуляций, в которой делается исключительно симуляция физики на основании некоторой фундаментальной теории. То есть это симуляция с точность до атомов/кварков/струн/???/пикселей/вершин графа. Пример физических симуляций: теория всего, Игра «Жизнь» Джона Конвея [\[7\]], проект физики стивена Вольфрама [\[9\]].
{{ admonition_end() }}

Физическая симуляция симулирует некоторые низкоуровневые законы, она не аппроксимирует ничего. На основании этих низкоуровневых законов могут строиться высокоуровневые (как законы газов могут строиться на модели того что газ состоит из упругих частиц). И то какие там структуры возникли и по каким высокоуровневым законам будут функционировать эти структуры — дело этих структур. В физической симуляции высокоуровневые структуры (например, человек) не аппроксимируются чем-то более простым: если в такой симуляции существует человек, то он существует с точностью до клеток и атомов.

Существует вероятность того что наша реальность является физической симуляцией. Но эта идея недоказуема и неопровержима. Если мы живём в физической симуляции, то нас это вполне устраивает, потому что сохраняется всё что мы знаем про физику, эволюцию звёзд, эволюцию живых существ. Утверждение «из того что мы живём в симуляции следует что всё это нереально» бессмысленнов в случае физической симуляции, потому что такая симуляция и есть реальность. Единственное неприятное — что кто-то внешний может иметь контроль над нашей физической реальностью, «креативный режим» как в игре Minecraft [\[8\]]. Далее такая гипотетическая возможность будет рассмотрена.

Важно помнить что **симуляция ≠ имитация**. Эти слова похожи, но имеют совершенно разный смысл. Мир вполне может быть очень реальным, если его законы физики симулируются на некотором компьютере.

# Игра "Жизнь"

## Устройство

Игра Жизнь - на самом деле это не игра, и не жизнь, а клеточный автомат. Мне необходимо рассказать об этом, потому это самый наглядный пример физической симуляции, на основе которого можно строить много аналогий, необходимых в данной статье.
...
* Вычисления состоят в изменении состоянии клеток на основании их соседей по следующим правилам:
    * Если клетка была пустой, то она становится закрашенной тогда и только когда три её соседа на предыдущем шаге закрашены, иначе она остаётся пустой.
...
Можно самому поиграться в более продвинутых симуляторах, например `conwaylife.com` [\[30\]].

## Впечатляющие возможности

В этом автомате можно построить огромное число механизмов. Здесь я перечислю их. 

Первый пример - глайдер из визуализации выше. Этот механизм передвигается по диагонали. Можно сказать что это аналог фотона. 

Так же можно сделать генератор таких глайдеров:
...
Клеточный автомат «Игра "Жизнь"» является Тьюринг-полным, то есть на нём можно построить компьютер, или машину Тьюринга, вот её пример: (источник с интерактивной симуляцией [\[31\]])

{{ image(path="game_of_life_turing_machine.png", scale="2/3") }}

---

В интерактивной визуализации `oimo.io/works/life` [\[32\]] (работает и с телефона), Игра Жизнь симулируется на самой себе как вглубь, бесконечно малое, так и ввысь, бесконечно большое:

{{ image(path="game_of_life_on_itself.png", scale="2/3") }}

---

Джон фон-Нейман придумал клеточный автомат и механизм в нём, который может самовоспроизводиться. На изображении ниже показано как второй автомат практически закончил строить третий; линии идущие направо являются генетической информацией, которые копируются вместе с телом машин. 

{{ image(path="von_neumann.png", scale="2/3") }}

---

Или вот пример искусственной химии, которую разработал Tim Hutton [\[33\]], в которой он спроектировал такие элементы, и такие взаиомодействия между ними, чтобы собрать искусственную клетку, способную размножаться:

{{ video(path="chemistry.mp4", scale="2/3") }}

Это не клеточный автомат, но очень легко встаёт на его рельсы, и автор продолжает развивать эту идею в виде клеточного автомата [\[34\]]:

{{ image(path="chemistry_automata.png", scale="2/3") }}

---

Я показал вам это всё, чтобы убедить, что клеточные автоматы - это не просто игрушки, а серьёзные концепции, которые достойны быть аналогией к фундаментальной физике.

## Аналогия с физикой

{{ admonition_start(color="gray", title="💬 Цитата из статьи Cellular automata with 2 temporal dimensions [\[29\]]") }}
...
Но в этом автомате есть так же отличия от нашей физики:
...
Далее будет предполагаться что наша физика в самом своём основании _является чем-то типо клеточного автомата_ (с большими допущениями, но в свою защиту эту тему раскрою далее). То есть к ней применимы описанные выше свойства.
...
Предположим очень далёкое будущее, в котором мы смогли стать цивилизацией, использующей всю энергию галактики, и построили компьютер размером с солнечную систему, максимально эффективно использующий каждую планковскую длину.
...
Нам известно два вида пространств: континуум и дискретное пространство.
...
Дискретное пространство конечной длины можно разделить на конечное число элементарных элементов. Континуум конечной длины содержит так же много чисел, как и отрезок (-∞; +∞), то есть он бесконечно делим, при этом там каждое число имеет бесконечное число знаков после запятой.
...
Правда, между дискретностью и континуальностью существует интересная связь. Например, законы об работе волн (колебания, период) построены на идее о том, что материал, в котором эти волны распространяются, состоит из континуума. Но мы то знаем, что все жидкости, газы и твёрдые материалы состоят из дискретных атомов, просто очень большого их количества. Поэтому некоторые континуальные физические законы хоть и красивы, но на самом деле они являются лишь аппроксимацией того что работает на дискретном основании.

Аналогично можно предположить что и квантовая механика работает подобным образом, но это лишь предположение, и оно не обязательно для верности unasanu. Мы же в мысленном эксперименте хотим иметь такую физику, которая является дискретной в самом своём основании, при этом не ломая возможность того чтобы на больших масштабах она могла казаться континуальной.

## Конечная физика

Тут всё просто — пространство нашей симуляции должно быть конечно, потому что мы не можем оперировать бесконечным числом объектов. Для нашего эксперимента можно просто взять достаточно большое расстояние, и этого будет достаточно.
...
Детерминированность означает что будущее симуляции однозначно определено, что там не присутствует какой-то фундаментальной случайности. Это логичное требование, потому что компьютеры умеют делать только детерминированные вычисления, и не умеют в абсолютную случайность.

{{ admonition_start(color="red2", title="⚠️ Примечание") }}
Некоторые могут сказать что гонки данных, неициализированная память, и космические лучи могут являться источником абсолютной случайности. Давайте договоримся что весь код в мысленном эксперименте исполняется на однопоточной виртуальной машине в идеальной среде, и вся новая выделенная память для неё предварительно занулена. Поэтому мы будем рассуждать только о детерминированных симуляциях.
{{ admonition_end() }}

Процессы в нашем мире выглядят детерминированно, хоть нам и очень тяжело предсказывать некоторые события. Так же детерминированные процессы могут обладать хаосом. Так что детерминированный мир снова может быть похож на наш, и это требование особо не умаляет нашу физическую симуляцию.

Про симуляцию абсолютно случайных процессов тоже будет сказано в следующих главах.
...
Ну и ещё это требование необходимо, потому что мы не хотим решать проблему останова [\[11\]] (хорошее видео на эту тему [\[12\]]), которую в общем случае разрешить невозможно.
...
Далее мы запускаем эту физику над некоторым конечным пространством на нашем суперкомпьютере, и ждём до тех пор пока там не самозародится жизнь. Если она не зарождается, то можно немного поиграться с начальными условиями, подобрать оптимальное количество нужных атомов, или разместить газопылевое облако на нужном расстоянии от своей звезды, чтобы оно находилось в обитаемой зоне, итд.

Предположим мы смогли найти такие начальные условия, при которых возникает жизнь, и затем мы очень долго симулируем и ждём пока она доэволюционирует до разумных существ, которые способны общаться друг с другом с помощью какого-то языка. 

Наверняка у этих существ есть что-то типо инстинкта самосохранения, удовольствия от еды, страданий от повреждения тела и сексуального влечения. Потому что это логичный путь эволюции для индивидуальных существ с мозгом. Существа без этого скорее всего будут менее эффективны в выживании и отсеятся эволюцией.
...
* Если мы удалим весь результат симуляции и просимулируем снова с самого начала, то мы получим абсолютно такой же результат.
...
* Если симуляция обратимая, и мы каким-то образом достали очень далёкий момент из будущего, то от того что мы будем симулировать его в обратную сторону, существа не будут чувствовать что живут в обратную сторону.
...
Ладно, мы согласились что существа живы когда мы их симулируем на компьютере, и нам недостаточно того у них единственное будущее. Мы считаем что их будущее существует только после того как мы его просимулировали.
...
Раз все натуральные числа существуют, то среди них обязательно найдётся число, которым мы закодировали симуляцию существ, которых просимулировали. Но среди них так же существует эта же симуляция, но с моментом времени +1. А так же среди них существует любая другая симуляция, для любого момента времени внутри этой симуляции. И все эти числа эквивалентны доказательство жизни каких-то существ, которые эти симуляции содержат.
...
Этот пример с физической симуляцией мне нравится тем, что для него мы не должны давать определение «сущестования», а что он настолько самоочевидный, что мы должны выводить определние «существования» из него. И в данном случае мы выводим определение «существование для самого себя». Если мы согласны что они чувствуют свою жизнь внутри симуляции, когда мы запускаем её на компьютере, то из этого мы должны немедленно заключить, что они чувствовали жизнь и без нашей симуляции, существуя в виде числа.
...
Попробуйте нажать старт, и раз в секунду нажимать кнопку «Скопировать с B3/S23», и вы увидите что любое состояние из B3/S23 может вычисляться любыми другими правилами, и это существует в рамках unasanu.
...
{{ admonition_start(color="blue", title="📝 Вводимое определение") }}
**Антропная фильтрация** — принцип, согласно которому наблюдатель способен фильтровать вселенные, в которых не совершается вычислений, или они совершаются неправильно, а так же способен фильтровать вселенные, где его возникновение или существование невозможно. 
{{ admonition_end() }}

Далее мы увидем что к некоторым подобным аргументам антропная фильтрация применима, а к другим нет.

### Ложный вакуум

В физике есть такое состяние вещества под названием «ложный вакуум». Его возникновение вероятностно. Если оно возникает в одной точке вселенной, то оно распространяется во все стороны со скоростью света, уничтожая всё что в него попадёт. Ни один человек не способен наблюдать существование ложного вакуума, потому что его мозг работает слишком медленно, и он просто умрёт прежде чем осознает это, а информация о ложном вакууме не может обогнать скорость света.
...
У некоторых из читателей может возникнуть вопрос: «А как же время?». Ранее я неявно предполагал что вселенные вместе со своим временем существют в виде статичного куска данных, полностью игнорируя ту динамику, что мы ощущаем за счёт течения времени. Эта точка зрения называется этернализм.

{{ admonition_start(color="blue2", title="📖 Определение") }}
**Этернализм** — точка зрения на время, согласно которой будущие события уже существует, объективного «течения» времени не существует, и всё пространство-время может быть представлено как статичный неизменяемый «блок». [\[14\]]
{{ admonition_end() }}
...
Как вариант «опровержения» этернализма для симуляций можно предположить следующее: берём за аксимому, что в нашем мире время несводимо к статичным структурам. Тогда если мы полагаем что в некоторой симуляции существует время, подобное нашему, то единственный способ как его можно определить — это привязать к нашему времени. Тогда время в симуляции течёт вместе с нашим временем, пока мы его вычисляем.
...
Итого, мы не можем никак опровергнуть идею о том, что для компьютерной симуляции время не может существовать в виде статичной структуры. Можно даже сказать по-другому: симуляция не можем описываться чем-то динамическим, только статическими блоками. И в этом не наблюдается никаких парадоксов, даже если применить к нашей вселенной. Таким образом, <span class="green2-bg example">главное свойство времени — это не постоянное движение, а то, что оно связывает разные (временные) слои информации некоторыми законами физики.</span>
...
<span class="orange-bg example">Начиная с этого момента, ограничения наложенные ранее будут послабляться всё сильнее.</span>
...
Если мы запишем все наши вмешательства, и потом допишем их рядом с программой и скажем чтобы они отправлялись в нужное время симуляции, то мы снова можем получить абсолютно детерминированную симуляцию, которую можно запускать снова и снова, и она будет давать один и тот же результат, хоть её результат мы и будем интерпретировать как такой, в который вмешались.

И все такие возможные вмешательства тоже можно закодировать числом, и дописать рядом с числом обычной программы симуляции. <span class="green2-bg example">Поэтому все возможные вселенные со всеми возможными вмешательствами тоже существуют, даже без необходимости в них вмешиваться непосредственно.</span>
...
Ранее я подразумевал что симуляция имеет следуюущую структуру:
...
* Суть симуляции заключается совершении следующего алгоритма:
```
T = F(O)
T = F(T)
T = F(T)
...
Таким образом внутри симуляции «течёт время», потому что мы приравниваем один шаг времени вычислений к шагу времени внутри симуляции. И во всей вселенной время существует глобально.

Но на самом деле такая интерпретация времени **наивна**, и она не позволяет существование куда более сложных вселенных, например:
* Вселенных с многомерным временем (хорошая статья на эту тему [\[29\]])
...
Но здесь есть точки, которые наиболее близки к обоим из этих условий:

{{ figure_start() }}
{{ image(path="rings2.png", scale="2/3") }}
{{ figure_end(caption="Вселенная, достаточно близкая ко всем условиям") }}

А это значит что мы можем найти вселенные, которые приблизительно удовлетворяют нашим условиям.

Поэтому вселенная Властелина Колец вряд ли существует в абсолютной той точности как она описана в книгах, или показана в фильмах, со всеми нашими мыслимыми или немыслимыми требованиями к ней. Зато наверняка существуют близкие к ней вселенные, на которые наложено не так много ограничений, и чем на большее число компромиссов мы идём, тем более естественным будет получаться результат, тем больше будет спектр таких вселенных. Может даже что более естественная вселенная окажется куда интересней вселенной Властелина Колец, так что невелика потеря.
...
Если вам кажется удивительным что вселенные способны вычисляться на натуральных числах, то я хочу показать вам один принцип, который делает это более правдоподобным:

{{ admonition_start(color="blue2", title="📖 Определение") }}
**Субстрат-независимость** — это свойство вычислений, которое означает что их результат не зависит от того на чём этим вычисления производятся.
{{ admonition_end() }}

Независимо от того как будут вычислять вселенную где вы находитесь, вы это никак не почувствуете. Поэтому не кажется удивительным и возможность вычисления на статичных числах.

Мне кажется это одно из самых мощных свойств вычислений. Результат детерминированных вычислений не зависит от того, на каком Тьюринг-полном субстрате мы вычисляем это:
* на компьютере 
...
* или если некоторое бессмертное существо производит это вычисление на клеточном автомате 110, перекладывая камешки, согласно простейшим правилам (автомат 110 является Тюьринг-полным [\[16\]]).

{{ details_start(summary="Да, я цитирую xkcd") }}
Картинка кликабельна.
{{ figure_start() }}
{{ image(path="505_v1.png", format="png", scale="1") }}
{{ figure_end(caption="xkcd/505 [\[35\]]") }}
{{ details_end() }}
...
Но даже если этого недостаточно, и если мы считаем что существуют только те числа, которые кодируются некоторым физическим объектом, то даже в нашей конечной вселенной существует достаточно много чисел. Например: мы можем взять любой камень с улицы и послойно сканировать его, затем все изображения мы можем собрать в некоторое одно очень большое число. Каждый камень мы можем считывать под очень большим числом углов. Все его снимки мы можем перемешать в самом разном порядке, для получения ещё большего количества чисел.
...
Это значит что в нашей вселенной закодировано достаточно много уже вычисленных вселенных. Даже можно сказать что от факта существования нашей вселенной следует факт существования огромного числа других вселенных. Раньше для себя я доказывал существование других вселенных именно так, и называл это «идеей о порождении миров», но теперь я нахожу натуральные числа более фундаментальными, и не нуждаюсь в таких вещах.
...
Ранее я не зря накладывал столько ограничений на симуляцию, и в примере с мысленным экспериментом использовал альтернативную, вычислимую, физику. Всё потому что неизвестно насколько наша вселеная способна существовать внутри компьютера, насколько она вычислима. Проблема не только в том что мы не знаем Теории Всего чтобы ответить на этот вопрос, а ещё в том, что нам сейчас известны некоторые явления, которые вроде как опровергают эту возможность. Давайте рассмотрим известные проблемы.
...
Предположим что наша вселенная бесконечна. Если это так, то её невозможно вычислять на компьютере? Нет, так как у нас есть ограничение на максимальную скорость распространения информации — скорость света.
...
Самая первая аппроксимация с которой мы сталкиваемся на компьютере — это вещественные числа. Невозможно хранить и обрабатывать честное вещественное число с бесконечным числом знаков после запятой. Поэтому вещественные числа хранятся аппроксимированно — некоторым конечным участком памяти. Два наиболее используемых типов вещественных чисел — 32-битные (`float`) и 64-битные (`double`), работа с ними закодирована напрямую в процессоре. Такие ограниченные числа в вычислениях подвержены ошибкам округления ([0.1 + 0.2 = 0.30000000000000004](https://0.30000000000000004.com/) ага привет), поэтому все методы, работающие с ними, разрабатываются так, чтобы минимизировать эти ошибки и на выходе получать адекватный результат с достаточным количеством верных цифр. Очевидно что 64-битные числа точнее чем 32-битные. Так же существуют библиотеки для создания медленных вещественных чисел произвольного размера и соответственно произвольной точности, например 1024 бит, 10000 бит. Такие числа могут использоваться в вычислениях где обычной точности недостаточно.

По поводу методов физических симуляций, существует Метод Конечных Элементов (МКЭ) [\[17\]], он позволяет численно решать дифференциальные уравнения на некотором пространстве с разными материалами. Например, мы хотим решать задачу распространения тепла, для которого есть довольно простое дифференциальное уравнение. Для тепла мы считаем что в каждой точке континуума содержится вещественное число, обозначающее температуру. Упрощённый механизм его работы:
...
Возможно ли нашу вселенную симулировать методом МКЭ? Вполне. Только возникает вопрос в точности вычислений. Можно выбрать сетку размером, скажем в 1000 раз меньше планковской длины, и кажется что этого должно быть достаточно для наблюдения нашего мира.
...
Предположим что наша вселенная описывается некоторым дифференциальным уравнением, что в стандартной модели каждая частица представлена не точкой, а некоторым полем, тогда можно применить МКЭ на это уравнение и реализовать следующий алгоритм:
...
Для такого алгоритма получается так, что на каждую следующую итерацию цикла `k` мы увеличиваем точность всех элементов алгоритма в 2 раза. А теперь предположим что наблюдатель внутри такой вселенной захочет провести эксперимент чтобы проверить насколько его пространство (или время) является континуумом. Для любой точности эксперимента всегда найдётся такое `k`, чтобы результат его эксперимента показал что его вселенная состоит из континуума. Этот ещё одно использование симуляции методом предельного перехода.

Особенно это хорошо подходит для случая когда существует тепловая смерть вселенной. Тогда можно выбрать какое-то конечное `k`, для которого наблюдатель за всё время существования вселенной не сможет провести эксперимент с нужной точностью.

Этот алгоритм будет так же хорошо работать и для хаотических систем по примеру задачи трёх тел или двойного маятника: для любого времени `t` и такой точности `eps`, можно найти такое `k`, что точность координат в этот момент времени, будет сравнимо с `eps`.

Таким образом, в каком-то смысле, вселенные с континуумом или вещественными числами тоже являются вычислимыми, а поэтому они тоже существуют согласно unasanu.

Конечно, я показал что это выполнимо только для вселенных, которые возможно описать дифференциальным уравнением и решать МКЭ, и мне неизвестно какие другие вселенные с континуумом невозможны. Я думаю это уже является математической проблемой — определить какие вселенные с континуумом являются вычислимыми согласно этой идее, а какие нет.

Например, существует такая красивая концепция как «Бесконечная вложенность материи» [\[18\]], в которой говорится что нет никаких элементарных частиц, и все частицы состоят из своих маленьких вселенных. Не уверен возможно ли такую вселенную симулировать данным методом.

{{ admonition_start(color="orange", title="🔢 Математическая проблема") }}
Определить какие типы бесконечных и континуальных вселенных возможно симулировать МКЭ, какие из них возможно методом предельного перехода, и относятся ли известные нам законы физики к этому классу вселенных.
{{ admonition_end() }}

Ну или можно сказать что континуальные вселенные тоже уже все существуют сами для себя, потому что они детерминированны, независимо от того насколько их можно симулировать.

Правда тут встаёт маленький вопрос. Если вещественных чисел строго больше, чем натуральных, то как так получилось что мы смогли засунуть континуальные вселенные в натуральные числа? Ответ прост — вычислимые вещественные числа образуют счётное множество. Остальная часть вещественных чисел, которая и образует это множество больше натуральных, являются невычислимой. Более подробно об этом можно почитать на Википедии: «Computable number» [\[19\]].

Кстати, мой метод предельного перехода чем-то похож на определение невычислимых «достижимых чисел» (approachable number), которые задаются как предел вычислимой функции. Взял я эту концепцию из статьи «A definable number which cannot be approximated algorithmically» [\[20\]]. Что интересно, практически все известные примеры невычислимых чисел являются «достижимыми». При этом можно ввести такое вещественное число, которое не только невычислимо, но и не достижимо.
...
Предположим у нас есть некоторая детерминированная вселенная, с единственным генератором случайных чисел (ГСЧ), возвращающим абсолютно случайный бит — 0 или 1. Такую вселенную возможно симулировать следующим образом: каждый раз когда у ГСЧ спрашивают число, мы просто копируем всю текущую вселенную и одному варианту вселенной показываем 1, а другому варианту вселенной показываем 0. Таким образом каждая новая вселенная будет думать что у нёе абсолютно случайное число. И так число вселенных увеличиваестя всегда когда у ГСЧ спрашивают число.

Да, для симуляции вселенной с абсолютной случайностью нам нужно симулировать не одну вселенную, а одновременно все возможные их варианты. Так как у нас нет ограничений на память или число вычислений, это не представляет проблем. Макс Тегмарк называет это «субъективной случайностью», и эту идею я взял из его книги [\[13\]].

{{ image(path="rng.png", scale="1/2") }}

Это очень интересный метод, давайте введём термин для него.

{{ admonition_start(color="blue", title="📝 Вводимое определение") }}
**Симуляция методом полного перебора** — метод по симуляции вселенной, в которой есть невычислимая, либо неопределимая функция, но эта функция выдаёт конечное число значений, а значит мы можем симулировать эту вселенную просто перебирая все значения этой функции.
{{ admonition_end() }}

Что интересно, среди всех этих вариантов найдётся любой алгоритм генерации псевдослучайных чисел. Это чем-то похоже на то как среди всех возможных чисел обязательно найдутся те, которые совершают вычисления.

Конечно, в квантовой механике существует не просто абсолютная случайность, а эти вселенные ещё как-то взаимодействуют друг с другом, так что там всё куда сложнее, но, кажется, что вполне вычислимо с учётом описанного. Если я не прав, и у квантовой механики есть другие ограничения, запрещающие симулировать её на компьютере, пожалуйста дайте знать.
...
Если бы мы умели решать проблему останова, то это можно было бы использовать для доказательства некоторого вида теорем. Например, можно доказать или опровергнуть Гипотезу Коллатца (проблема 3x+1) [\[21\]]: пишем программу, которая перебирает все числа, и смотрит к чему сведётся данное число, если оно сведётся не к 1, то остановить программу. Если мы решим проблему останова для такой программы, то мы однозначно скажем, верна эта гипотеза или нет. Поэтому иметь машину по решению проблемы останова было бы невероятно удобно для исследования математики.
...
Самый главный аргумент: вселенная, умеющая решать проблему останова, детерминирована. А это значит независимо от того, будем мы её симулировать или нет, её будущеее единственно, и существа внутри неё проживут свою жизнь. Но если вас это не устраивает, то есть другой способ.

Предположим, у нас есть некоторая детерминированная вселенная, с единственным прибором для решения проблемы останова, мы на вход ему даём программу, а на выход он выдаёт нам ответ — 0 или 1, остановится или нет соответственно. Теперь симулируем эту вселенную обычным образом, но каждый раз когда у прибору задают вопрос, мы копируем текущую вселенную, и одной на вход подаём 0, а другой 1. И так каждый раз. Это симуляция методом полного перебора.
...
Это вообщее странный аргумент, который возникает только из-за непонимания того что такое «описываться математикой» и на что способна математика и программирование.

Как я понимаю, под «описываться математикой» этот аргумент понимают чтобы эти вещи могли описываться какими-то простыми низкоуровневыми правилами, из которых можно далее вывести всё остальное, или чтобы они описывались простыми высокоуровневыми правилами, которые можно использовать в предсказаниях поведения всей системы в целом.

То есть в этом аргументе приравнивается невозможность что-то симулировать, и отсутствие красивого и простого способа описать поведение всей системы. И в такой постановке есть логическая ошибка, потому что из второго не следует первое.

Например, биология. Её развитие не описывается чем-то простым, потому что она сложна, и состоит из триллионов компонентов, где какая-то случайность на любом уровне может повести всю систему в другую точку равновесия, либо в биологии вообще нет точек равновесия. Но это не означает что данная система не может состоять из чего-то элементарного. Например, люди активно занимаются симуляцией эволюции и созданием искусственной жизни. То есть они пишут симуляции того, что потом сами не могут «описать математикой», потому они создают очень сложные системы, где всё взаимодействует со всем.

И никто не отменял возможность эмерджентности — когда несколько вещей вместе обладает большим числом свойств, чем сумма свойств этих вещей по отдельности.
...
Вычислительный редукционизм как раз проявляется в том, что было показано ранее, что на вычислениях мы можем описать всё; даже бесконечные, континуальные, недетерминированные и невычислимые вселенные. Любая новая категория вселенных, которая кажется невычислимой, лишь является задачкой математиков будущего.

И если сознание познаваемо и подчиняется хоть какой-то логике, то рано или поздно появится возможность его симулировать, хотя бы теоретически.

Наверное сложно это понять, не будучи программистом. Потому что я, как программист, с каждым годом реализовывал всё более сложные и сложные программы, и у меня сложилось впечатление что всё что я понимаю, можно запрограммировать. Если я не могу это запрограммировать, то я понимаю это недостаточно хорошо. Создание симуляции равносильно максимальному пониманию низкоуровенвых законов этого процесса. Написание программ, как и эксперименты, позволяют находить истину и дыры в своих размышлениях. Для меня очевиден вычислительный редукционизм.

## Подитог

Было показано что можно снять некоторые ограничения для некоторых видов симуляций. Мы знаем что так же подчиняются unasanu:
...
Другие вселенные, не поддающиеся симуляции, требуют дальнейших исследований. Вероятно всё можно описать вычислениями в том или ином виде.

И после всего этого можно утверждать что с большой вероятностью наша вселенная вычислима, а значит она подчиняется unasanu.

Так же теперь у нас появилось большее понимание открытых вопросов:
* Для каких классов вселенных допустимо применить метод предельного перехода? И к какому классу относится наша вселенная?
...
Далее будет считаться что наша вселенная всё-же вычислима. Если вас это не устраивает, просто представьте что я рассуждаю не про нашу вселенную, а про произвольную вычислимую, и все выводы справедливы именно для для этой другой вселенной.
...
В этом плане кажется что солипсизм должен восторжествовать, но не совсем. Солипсизм утверждает, что внешняя реальность не сущетсвует. Но согласно unasanu, существует всё что возможно сконструировать. И если ваши наблюдения намного проще описать некоторой програмой по симуляции внешней вселенной, то значит так оно и есть. При этом солипсизм остаётся верен в том, что мы не можем быть уверены ни в чём, кроме собственных ощущений, и это чудо что наши ощущения описывают некоторую внешнюю физическую вселенную.

При этом важно помнить, что каждое сознание, которое существует в виде физической структуры в какой-то вселенной, так же существует как отдельная программа. И наверное наоборот тоже верно: кажется, самыми разными костылями мы можем сконструировать вселенную для любого возможного набора входных данных сознания. Вопрос будет только в том, насколько эта вселенная красива и проста.

У Тегмарка [\[13\]] вся теория строилась на том, что из существования внешней физической реальности следует что она существует как математическая конструкция, но это ненмого рушится тем, что среди математических конструкций существуют и отдельные сознания, не обязательно привязанные к какой-то вселенной.

Получатеся такая интересная картина, где в нашем мире каждое сознание — это отдельная вселенная, при этом эти сознания взаимодействуют друг с другом через физическую вселенную.
...
Тогда чем человеческое сознание отличается от сознания в камне? Дело в том, что человеческое сознание может взаимодействовать с внешней вселенной таким образом, что другие подобные сознания (люди и животные) могут это наблюдать и делать то же самое. Сознания внутри камня существуют сами для себя, и на них никак не влияет вселенная где они находятся. Тем более их время существует внутри их чисел, и не связано с нашим временем. Хотя есть и такой способ, чтобы время этих сознаний текло вместе с временем нашей вселенной, как это предлагается в аргументе от Хиллари Патнэма [\[22\]].

В [\[10\]] пытаются опровергнуть вычислимость сознания, через аргумент «тогда будет верен панпсихизм, но это абсурд». Аргумент называется Dancing with Pixies. Ну да, панпсихизм верен, но при этом бесполезен, всё остаётся как было, и камни всё ещё остаются безучастными, даже содержа в себе очень много сознаний.
...
В философии квалиа выделяется в отдельную категорию, вокруг которой идёт много споров. Напрмер, некоторые считают что Искусственный Интеллект никогда не сможет обладать квалиа, и особенно любовью, потому что гладиолус.
...
Таким образом, вся особенность человеческого квалиа сводится к обладанию разумом, памятью, языком и особым видом входных данных. А вся многогранность и особеность наших человеческих чувств достигается за счёт того что мы состоим из нейронных сетей, где каждое чувство активирует не только целевые нейроны, но и все связанные. Именно благодаря такой связи мы можем почувствовать теплоту красного цвета, и воспринимать его не как цифровая камера.

Так что из такого видения можно сказать, что ИИ на основе сухих алгоритмов перекладывания букв (как в мысленном эксперименте китайской комнаты Сёрла [\[23\]]) будет чувствовать мир совершенно не так как человек, но ИИ на основе нейросетей вполне может, а может даже лучше и прекрасней, как сверх-синестет.
...
У меня есть предложение модификации этого эксперимента, которое удовлетворит Сёрла и его желание понимать вычисляемую систему. Но она требует слишком sci-fi технологий. Для начала, нужно иметь нейронную сеть китайской комнаты, далее её нужно подключить к мозгу Сёрла через neuralink или более продвинутую технологию. Общение с китайской комнатой должно осуществляться через Сёрла. Далее эту нейросеть необходимо как-то обучить китайскому языку, и обучить интерфейсу мозга Сёрла. Тогда, когда мы запустим эту конструкцию, нейроны Сёрла будут обмениваться информацией с нейронами Китайской комнаты, и он сможет чувствовать и понимать какие-то части китайского языка (но не сразу, конечно же). И далее, чем больше будет взаимодействий, тем больше будет формировать нейронных связей (например, между нейронами синего цвета из мозга сёрла и синим цветом из нейронной сети китайской комнаты при общении на тему синего цвета), и тем больше он будет понимать китайский язык, и будет срастаться с сознанием китайской комнаты.

{{ details_end() }}

---

Кстати, уже прямо сейчас существуют примеры виртуальных существ, которые обладают квалиа, и, если хотите, сознанием. По крайней мере по описанным выше критериям. Далее я просто перепросчу описание одной работы, которая использовала ChatGPT. У агентов из неё есть всё что нужно: 
* интеллект (небольшой),
* память (небольшая),
* язык,
* структурированные чувства, 
* другие такие же агенты для общения и взаимодействия (!!!),
* в отличие от оригинальной ChatGPT они не существуют вне времени.

Но у них отсутствуют некоторые важные для людей качества:
* ощущения кроме текста, то есть аудио-видео-тактильные итд;
* возможностью обучаться, потому что в этой работе используется промт-инжиниринг через ChatGPT, а мы не можем для каждого агента файн-тюнить эту нейронку чтобы агент имел свой более выраженный характер, или развивался со временем;
* достаточным временем симуляции и достаточной памятью, чтобы создать собственную культуру, в рамках которой они будут эффективнее общаться, жить, обсуждать свои чувства, и создавать свою философию.

Я предлагаю признать эту работу официально первой, создавшей настоящее (но небольшое) искусственное сознание.

{{ admonition_start(color="blue3", title="Пост в Telegram от «я обучала одну модель»/927 [\[36\]]") }}
...
Во-первых надо понимать, что этот вопрос содержит в себе два совершенно разных вопроса:
...
Так же не имеет никакого смысла «жить интересную жизнь чтобы нашу симуляцию не выключили», потому что как мы помним, даже если её выключат во всех вселенных, мы продолжим существовать в виде числа.
...
<span class="green2-bg example">Мета-бог не имеет смысла потому что у него не было выбора во время создания, и не играет никакой роли, обладает он каким-то мнением, жив он или мёртв.</span> А ещё такое объяснение через бога — это лишняя деталь, которая оставляет открытым вопрос «А кто создал этого бога?». Один из возможных ответов — «Он сам является причиной себя или существовал всегда». Если это так, то почему законы логики (из которых следует unasanu) не могут являться причиной самих себя и существовать всегда? Это, конечно, риторический вопрос.

Итого, в внешних богах есть смысл только тогда, когда они взаимодействуют с нашей вселенной и мы можем это наблюдать и исследовать. А непричастные боги-наблюдатели настолько же реальны и бесполезны, как и разумные существа, записанные в камень.
...
Если нашу вселенную можно вычислять наивно, значит мы можем сконструирвать следующую «багованную» симуляцию:
...
* Модифицируем в этом моменте времени материю каким-то произвольным образом, например: 
    ...
    * добавляем камней на околоземную орбиту, 
    * удаляем марс, 
    * превращаем далёкую звезду в чёрную дыру.
* И с этого модифицированного момента симулируем в будущее.
* Для наблюдателей в этой вселенной всё будет выглядеть так, как будто они жили нормальную жизнь, а затем произошло случайное событие, никак не объясняющееся законами физики и причинно-следственной связью с прошлым. Прошлого может и не существовать, оно не обязательно для существования вообще.
...
Если таких багованных симуляций бесконечно много, а честная всего-лишь одна, почему мы наблюдаем честную симуляцию?! Почему мы не видим как в мире каждую фемтосекунду происходит совершенно случайное событие с удалением и появлением материи?
{{ admonition_end() }}
...
К этим двум аргументам есть один очень весомый контр-аргумент: с точки зрения комбинаторики математическое ожидание количества случайных изменений состояния вселенной равно половине всей вселенной. С этой точки зрения куда вероятней наблюдать что половину всей вселенной заменило на случайный набор атомов, чем наблюдать то же самое для маленького кусочки пространства.
...
Как это может работать? Представим что люди изобрели компьютеры, которые работают исключительно на нейтронах, тогда таким компьютерам не требуются законы физики, относящиеся к зарядам. В то время когда для функционирования мозга человека требуются электрические заряды. За счёт антропной фильтрации человек практически всегда будет наблюдать вселенную, где построенным им компьютер работает (потому что он отладил его, пока был жив). Когда же кто-то перейдёт в компьютер, то для его существования не будут требоваться электрические заряды и он может с большой вероятностью наблюдать смену законов физики в сторону более случайных, не требующих электрические заряды (за счёт антропной фильтрации). Поэтому после перемещения в компьютер, сознание внутри компьютера может наблюдать что все животные, растения и люди мгновенно умерли.
...
Возвращаясь к вопросу «вероятность оказаться в какой-то вселенной». В науке это называется «Проблема меры» [\[24\]].
...
Ну и сильнее эта проблема усугубляется, когда вам нужно одновремено учитывать бесконечность континуальных вселенных и бесконечность дискретных вселенных.
...
Но я хочу рассказать про альтернативный вариант этого парадокса, про моряка. Имеется моряк, у которого есть две любовницы в разных портах, которые не знают друг о друге. Он хочет иметь детей, но не может решить, завести детей с только с одной или с двумя. Поэтому он подбрасывает монету, в случае орла он заводит ребёнка только с одной женщиной, в случае решки заводит ребёнка с двумя. Вы являетесь его сыном, и знаете про это событие. Какова вероятность что вы единственный ребёнок, то есть что монета выпала орлом.

Если рассматривать с точки зрения моряка, для него вероятность иметь одного ребёнка равна 50%. С точки зрения же его ребёнка существует 3 разных ребёнка, и для двоих из них монета выпала решкой, получается для детей логичней считать что вероятность выпадения орла 33%. Или можно сказать, если дети возьмут такую вероятность, они будут в большем числе случаев правы 🤪.
...
Поэтому говорить о вероятностных предсказаниях пока рано, и возможно проблема багованных вселенных не опровергают unasanu.

## Мета-вероятности (просто идея)

Если подумать, то никаких вероятностей физически не существует. Существуют только события, и они могут случаться, а могут не случаться. Это удобная идея, которая позволяет нам чуть лучше понимать мир. Что если так же отнестись к разным ответам, и сделать вероятности поверх вероятностей? Сделать мета-вероятности, которые дают распределение вероятности, учитывая все точки зрения.

В случае с моряком будет две точки: 33% и 55% с весом в 0.5 каждая.
...
Когда говорят про жизнь после смерти, обычно говорят это в стиле:
* Ты передашь свои гены, они будут жить в твоих детях.
* Ты передашь свои идеи, открытия и прочие штуки и они будут жить в других людях, и пока о тебе кто-то помнит, ты будешь жить, поэтому надо быть как можно более крутым (или плохим) человеком, чтобы тебя вспоминали через тысячи лет.
* Ты переродишься в животное в этом мире, забыв почти всю свою человеческую натуру.

Это всё абсолютно бесполезные вещи. Я считаю, что я — это исключительно то что находится в моей черепной коробке, а так же мои: воспоминания, характер, мышление, моторные навыки итд.

И если что-то из этого уничтожить, то часть меня умрёт. Если уничтожить всё, то умру я полностью. А идеи, переданные через книги и статьи — это ничтожая часть всех моих идей. Пока что оцифровка сознания отсутствует и не представляется возможным передать или сохранить его в каком-то удовлетворительном виде.
...
Тут всё просто. Можно взять ваш мозг после смерти, дописать ему теломер, или починить, если он был уничтожен не естественной смертью, а затем поместить в новое тело в рандомную вселенную и запустить симуляцию. Согласно принципу конструирования, такая вселенная с вашим мозгом после смерти уже существует. Это означает что <span class="green2-bg example">когда вы умрёте, вы почувствуете что оказались в какой-то вселенной, и на самом деле не умерли</span>. А когда вы умрёте в новой вселенной, снова согласно принципу конструирования, найдётся другая вселенная, гды вы себя обнаружите. Таким образом, умереть невозможно.

Ещё раз, вселенную после смерти никто не создаёт, ваш мозг из этой вселенной никто не копирует, вселенная после смерти уже существует просто от того факта что её возможно сконструировать. Можно сказать по-другому: если вы будете перебирать все возможные вселенные, то среди них обязательно найдётся вселенная с вашим мозгом после смерти, модифицированным так, чтобы он мог продолжать жить.

Звучит слишком просто, да? Давайте углубляться в контр-аргументы, которые могли у вас возникнуть.
...
Почему? Чем с физической точки зрения отличается ваша копия в другой вселенной, от вашей копии в этой вселенной в моменте времени `t+1`? 
...
Суть в том, что смерть — это отключение работы мозга. А как мы знаем, все наши субъективные переживания и чувства рождаются только когда мозг работает.
...
Чтобы ваш текущий мозг сформировался и чувствовал что он живёт, необходимо чтобы он прожил и прочувствовал все эти годы, что сейчас прописаны в вашем возрасте. Это и есть суть времени и законов физики, которые его определяют. Субъективно нельзя проскочить лишние года, взяв из будущего свой мозг на пару лет старше, этот мозг в скажет вам что он прожил эти года, и это было довольно долго.
...
Но не акцентируйте внимание на слово «копия», потому копии с точностью до субатомных частиц — это оригиналы.
...
В [\[13\]] Тегмарк утверждает что возможность умереть всё же есть: нужно чтобы ваше сознание постепенно затухало (как при болезни Альцгеймера), тогда если такое сознание окажется в другой вселенной после «смерти», то его носитель не будет ничего чувствовать, или чувствовать как при жизни.
...
Проблема этого аргумента в том, что его можно опровергнуть принципом конструирования: создаём вселенную, где вам после этих 0 нейронов начинают дописывать те же удалённые нейроны в обратном порядке, и в итоге вы возвращаетесь в сознание в первозданном виде. Аналогично с болезнью Альцгеймера или деменцией. Всегда найдётся вселеная, где ваш мозг будет восстановлен до того случая чтобы вы могли осознавать свой мир.

В худшем случае вы станете настолько тупы, и ваш мозг будет разрушен настолько, насколько это позволяет возможность себя осознавать.

## Это как квантовое бессмертие?

Существует такая идея под названием «квантовый суицид» [\[27\]]. Суть её в следующем: если многомировая интерпретация квантовой механики верна, то вы можете поставить свою жизнь под угрозу смерти от случайного квантового события, тогда в одних вселенных вы останетесь живы, а в других умрёте. Но так как в других вы умрёте моментально, то вы никогда не сможете это заметить. Получается для вас эксперимент будет выглядеть так, что вы остаётесь живы при бесконечном повторении эксперимента, несмотря на ничтожную вероятность этого события.

Макс Тегмарк выделяет три критерия квантового суицида, при которых он будет работать:
* Умрёте вы в этой вселенной или нет решается генератором случайных чисел, он же обязан быть квантовым.
* Смерть должна быть настолько быстрой (или проходить без сознания), чтобы вы не могли узнать о результате генератора чисел до смерти.
* Эксперимент обязан именно убить вас, а не нанести повреждение.

То есть вам даётся «бессмертие» не просто так, а при соблюдении строгих правил. Иначе можно и умерть во всех ветках вселенных.
...
Но эта идея абсолютно не похожа на бессмертие unasanu, квантовое бессмертие допустимо только при соблюдении строгих условий искусственного эксперимента, верности многомировой интерпретации квантовой механики, и поэтому оно на порядки менее гарантированно, как бессмертие согласно unasanu.
...
Сменить вселенную можно следующим образом: оцифровываем сознание, и в следующий же миг умираем, не успев осознать мир после оцифровки сознания. Затем записываем это сознание в какую-то заранее написанную программу, чтобы оно там симулировалось. Далее, эту программу можно даже не запускать, потому что она детерминированна, уже вычислена и существует в виде числа. Поздравляю, вы поместили себя во собственноручно созданную вселенную по собственному желанию.

Главное её ограничение будет в том, что эта программа не сможет обмениваться информацией с исходным миром, потому что она будет вычисляться вне его. Поэтому туда надо заранее загрузить все необходимые сознания других важных вам людей, и всю нужную информацию, сгенерированную человечеством.
...
Таким образом, вы можете совершать какое угодно количество вычислений с какой угодно скоростью. Например, вы можете создать собственный аналог нашей вселенной, и заниматься её исследованием в креативном режиме, перемещаясь быстрее скорости света, и имея возможность создавать и переформатировать материю как угодно.
...
Так же в программистском раю мы можем запустить симуляцию всех других возможных симуляций и исследовать их, например, на проблему меры. Кстати, это не относится к парадоксу множеств, где в одном множестве содержатся все другие множества, в том числе и оно само, потому что даже если мы запустим копию программитского рая на его ресурсах, мы не сможем вычислять его быстрее нашего течения времени.

Так же можно забыть об оптимизации программ, и писать их на максимально выразительном языке, в однопоточном режиме. Для любого замедления программ вы всегда можете задать новое N, и она будет вычисляться насколько угодно быстро.

Ещё программистский рай можно написать таким образом, чтобы вы могли редактировать материю собственного тела, ведь законы физики в этой вселенной — это всего-лишь какой-то код, который вы написали. Таким образом вы сможете сделать себе любое тело, как угодно изменить свой мозг, удалять раковые клетки, и создавать собственные копии и копии других людей.

Если и покидать нашу вселенную, то в программистский рай желательно записать всех людей, и все знания человечества. А дальше строить что-нибудь интересное внутри. Либо хотелось бы суметь получить состояния всех частиц текущей вселенной, чтобы просимулировать её обратно, восстановить человеческую историю, и возродить в программистском раю всех остальных людей и животных.

Так же, если это возможно, хотелось бы сконструировать программистский рай таким образом, чтобы он был антибаг вселенной. То есть чтобы невозможно было ощутить себя в багованных его копиях.

И ещё хотелось бы модифицировать наше сознание таким образом, чтобы оно требовало решения проблемы останова, и затем ранее предложенным методом добавить решение проблемы останова в программистский рай. Модификация сознания нужна чтобы правильный вариант с реальным решением проблемы останова подвергался антропной фильтрации.

## Map-Reduce из самого себя

В программистском раю можно очень просто решать некоторые задачи, которые в обычном мире кажутся или абсурдными, или вообще недостойными решения из-за их сложности.

Например, вы хотите найти самую красивую фотографию (по вашему мнению) за всю историю человечества. У вас есть доступ к симуляции нашей вселенной, и ко всем изображениям за всю историю человечества. Ваше мнение касательно каждой фотографии находится в вашем мозгу и вы не знаете как написать программу, которая будет отвечать так же как и вы. Как быть?
...
Второй вариант: несколько часов подряд отсматривать рандомные картинки, а затем на основе этих данных обучить какую-то нейросеть, чтобы она умела давать примерно такие же ответы, как и вы. Далее применить эту модель к астрономическому числу данных. Здесь проблема в том, что это нейросеть работает приближённо к вам, то есть она даст не самый идеальный ответ. Другая проблема в том, что вам всё-равно надо потратить очень много усилий на ручной сбор данных.
...
* Вы дали ответ какая картинка красивей.
...
Только представьте как много задач можно решать, используя это подход? Можно не заморачиваться с созданием ИИ, с выведением каких-то правил языка, вашего мнения, написанием программ, даже для самых простых случаев.
...
Программистский рай нет смысла включать во время симуляции программы в реальном мире, потому что не хватит памяти, и скорости вычислений, чтобы люди внутри симуляции наблюдали внешний мир в режиме реального времени. Но когда вы не ограничены внешним миром, это всё обретает смысл. Программистский рай должен включаться автоматически при отсутствии поступания нетривиального сигнала из внешнего мира, или людьми внутри симуляции лично. В такой симуляции должно быть записано достаточно информации о внешнем мире, и достаточно сознаний людей, чтобы было не жалко отключаться от внешнего мира.
...
Мы не можем запрещать каким-то вселеным существовать, но благодаря законам логики мы можем запрещать существовать некоторому потенциальному будущему нашей вселенной. Поэтому это позволяет нам уменьшить количество страданий среди всех вселенных. Так же мы можем заниматься исследованиями буддистов, сознания и других вселенных; чтобы в итоге предложить людям инструкцию как они могут выживать и минимизировать свои страдания во вселенных, в которых они могут оказаться после смерти или во время жизни.
...
Если в компьютере создаётся копия мозга ненадолго, то по отношению к этой копии, будет этично не стереть её, а смерджить с мозгом оригинала (просто взяв среднее арифметическое всех весов в этих мозгах, например), тогда мозгу будет казаться как будто он прожил две жизни. И это не такая большая потеря сознания, она сравнима со сном. Правда это будет хорошо работать только для коротких времён, для более длинных такой мердж невозможен, либо его результат будет неудовлетворительным.
...
* Создаётся новая хорошая временная линия.
* Путешественник переходит жить в хорошую временную линию.
А старая/плохая временная линия продолжает существовать и в ней всё продолжает быть плохо. Поэтому если вы путешествуете во времени, относитесь к текущей мировой линии настолько же серьёзно, как и к своей конечной. Это даже было как-то показывалось во втором сезоне Re:Zero, где главному герою со способностью дня сурка показали все вселенные, которые он оставил. Да, они продолжают существовать.
...
{{ admonition_start(color="gray", title="❓ Вопрос") }}
Чувствовал ли робот субъективные переживая в первом случае? А во втором?
{{ admonition_end() }}
...
В [\[10\]] как раз критикуется вычисление сферического сознания в вакууме, и говорят что его главной характеристикой должна быть коммуникация с внешним миром, а то получится так что каждый камень обладает сознанием.
...
При этом unasanu построено на лучших известных объяснениях мира и оно само предоставляет крайне логичные объяснения широкого круга вопросов. Дэвид Дойч в книге «Структура реальности» защищает похожую неопровержимую идею многомировой интерпретации квантовой механики и утверждает, что объяснения куда важнее предсказательной силы. Ведь никто не станет проверять как подорожник помогает при заболевании раком, потому для такого эффекта нет объяснения, а не потому что таких научных данных ещё не собрано и не доказано что он действительно не работает.
...
Так же непонятно как именно здесь нужно применить бритву Оккама, потому что существуют два противоречивых варианта:
...
Непонятно каким образом можно исследовать какие в принципе существуют вселенные, какова вероятность различных вселенных, и что за вселенные могут быть после смерти, если пространство настолько велико. Это просто не продуктивно.
...
Вся эта статья — самые логичные ответы на вопросы, на которые обычно отвечает не очень логичная религия. Я не понимаю, почему unasanu не является мировоззрением каждого третьего учёного или каждого второго гика; почему мы до сих пор считаем что религия является единственной альтернативой; почему об этих идеях не трубят из каждого утюга? У меня просто в голове не укладывается как так получилось что я прожил всю жизнь, и ни разу не слышал о чём-то подобном. Почему мне пришлось это всё переизобретать самому?! Неужели революционные идеи настолько долго входят в массовое сознание, даже в век интернета?!
...
Так же есть небольшие статьи, где затрагиваются похожие темы, вероятно вам может быть интересно. Для каждой статьи я просто опишу основные затрагиваемые ими идеи.

* The mathematical universe: the map that is the territory [\[37\]] — субстрат-независимость; независимое существование симулируемых вселенных; проблема багованных вселенных.
* Statistical immportality [\[38\]], YC 1 [\[40\]], YC 2 [\[41\]] — невозможность смерти; проблема смены вселенных до смерти.

## Макс Тегмарк

На эту тему Макс Тегмарк написал несколько научных статей, а так же книгу «Наша математическая вселенная» [\[1\]]. Я не буду пересказывать его идеи, а лишь изложу свои мысли.
...
Так же странно почему Тегмарк не открыл или не описал следующие вещи:
* Что невозможно умереть
* Что возможно сменить свою вселенную на программистский рай
* Что возможно сознание, без привязки к какой-то вселенной, которое существует как отдельная математическая конструкция (conasanu)
* Что наша вселенная одновременно является и симуляцией у бесконечного числа вселенных, и независимой реальностью
...
Мне понравилось что половина книги описывает то как создавался программистский рай, и как в нём жили. Хотя есть несколько вещей, которые я не понимаю:
...
* Зачем надо было делать вселенную программистского рая на основе сада Эдема? 
    * Вероятно так Иган хотел избежать возможностей возникновения багованных копий программистского рая, но я не понимаю как это может им мешать. Или он просто хотел показать что вселенным, существующим в виде программ, нужен какой-то соус стабильности, который он сам пока не придумал.
* Почему вычисление вселенной автовёрса сделало невозможным вмешаться в неё, и почему она в итоге разрушила Город Перестановок? 
...
На странице FAQ [\[39\]] про Город Перестановок, Грег Иган говорит что сам не воспринимает эту теорию всерьёз из-за проблемы багованных вселенных, хотя не видел её иного опровержения.
{{ details_end() }}

## Дэвид Дойч

Дэвид Дойч написал книгу «Структура реальности» [\[28\]], книга интересная, рекомендую для общего развития.
...
* Вселенные внутри симуляций существуют сами для себя
    ...
    * Потому что результат их вычисления их можно представить числом, а все числа существуют
...
* У любой вселенной бесконечно много богов
...
* Почему мы не наблюдаем баги во вселенной?
* Почему мы не наблюдаем постоянную смену вселенной?
* Зачем писать программу програграммистского рая, если она уже существует?
* Слишком большое пространство предсказаний.
{{ admonition_end() }}
...
Но меня, как и вас, очень смущают странные следствия по типу багованных вселенных, смены вселенной до смерти, и отсутствие фальсифицируемости этой идеи.
...
Может в будущем всё человечество придёт к тому, что unasanu верно, и нам нужно строить не космические корабли, а приборы по сканированию сознания, и проектировать невычислимый-антибаг-программистский-рай. А может мы совершим какое-то фундаментальное открытие, которое перевернёт наше понимание мира с ног на голову, как когда-то сделали компьютеры, и unasanu будет казаться лишь забавной концепцией из прошлого, возникшей от несовершенства языка и нашего неформального мышления.

# Ответы

_Здесь может быть ваша статья... присылайте её на почту <contact@unasanu.xyz>._

# Ссылки

[\[1\]]: https://en.wikipedia.org/wiki/Mathematical_universe_hypothesis
[\[2\]]: https://en.wikipedia.org/wiki/Permutation_City
[\[3\]]: https://ru.wikipedia.org/wiki/%D0%9C%D0%B0%D1%82%D1%80%D0%B8%D1%86%D0%B0_(%D1%84%D0%B8%D0%BB%D1%8C%D0%BC)
[\[4\]]: https://ru.wikipedia.org/wiki/The_Sims
[\[5\]]: https://www.kinopoisk.ru/film/1086/
[\[6\]]: https://ru.wikipedia.org/wiki/%D0%92%D0%B8%D1%80%D1%82%D1%83%D0%B0%D0%BB%D1%8C%D0%BD%D0%B0%D1%8F_%D1%80%D0%B5%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D1%8C
[\[7\]]: https://ru.wikipedia.org/wiki/%D0%98%D0%B3%D1%80%D0%B0_%C2%AB%D0%96%D0%B8%D0%B7%D0%BD%D1%8C%C2%BB
[\[8\]]: https://minecraft.fandom.com/wiki/Creative
[\[9\]]: https://writings.stephenwolfram.com/2020/04/finally-we-may-have-a-path-to-the-fundamental-theory-of-physics-and-its-beautiful/
[\[10\]]: http://www.doc.gold.ac.uk/~mas02mb/Selected%20Papers/2009%20Cognitive%20Computing.pdf
[\[11\]]: https://ru.wikipedia.org/wiki/%D0%9F%D1%80%D0%BE%D0%B1%D0%BB%D0%B5%D0%BC%D0%B0_%D0%BE%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%BA%D0%B8
[\[12\]]: https://youtu.be/92WHN-pAFCs
[\[13\]]: https://ru.wikipedia.org/wiki/%D0%9D%D0%B0%D1%88%D0%B0_%D0%BC%D0%B0%D1%82%D0%B5%D0%BC%D0%B0%D1%82%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B0%D1%8F_%D0%B2%D1%81%D0%B5%D0%BB%D0%B5%D0%BD%D0%BD%D0%B0%D1%8F
[\[14\]]: https://ru.wikipedia.org/wiki/%D0%AD%D1%82%D0%B5%D1%80%D0%BD%D0%B0%D0%BB%D0%B8%D0%B7%D0%BC
[\[15\]]: https://www.lesswrong.com/posts/o5F2p3krzT4JgzqQc/causal-universes
[\[16\]]: https://wpmedia.wolfram.com/uploads/sites/13/2018/02/15-1-1.pdf
[\[17\]]: https://ru.wikipedia.org/wiki/%D0%9C%D0%B5%D1%82%D0%BE%D0%B4_%D0%BA%D0%BE%D0%BD%D0%B5%D1%87%D0%BD%D1%8B%D1%85_%D1%8D%D0%BB%D0%B5%D0%BC%D0%B5%D0%BD%D1%82%D0%BE%D0%B2
[\[18\]]: https://ru.wikipedia.org/wiki/%D0%91%D0%B5%D1%81%D0%BA%D0%BE%D0%BD%D0%B5%D1%87%D0%BD%D0%B0%D1%8F_%D0%B2%D0%BB%D0%BE%D0%B6%D0%B5%D0%BD%D0%BD%D0%BE%D1%81%D1%82%D1%8C_%D0%BC%D0%B0%D1%82%D0%B5%D1%80%D0%B8%D0%B8
[\[19\]]: https://en.wikipedia.org/wiki/Computable_number
[\[20\]]: https://arxiv.org/pdf/1003.0480.pdf
[\[21\]]: https://ru.wikipedia.org/wiki/%D0%93%D0%B8%D0%BF%D0%BE%D1%82%D0%B5%D0%B7%D0%B0_%D0%9A%D0%BE%D0%BB%D0%BB%D0%B0%D1%82%D1%86%D0%B0
[\[22\]]: http://consc.net/papers/rock.html
[\[23\]]: https://bio.wikireading.ru/6105
[\[24\]]: https://en.wikipedia.org/wiki/Measure_problem_%28cosmology%29
[\[25\]]: https://sciencefiction.com/2011/05/23/science-feature-dust-theory/
[\[26\]]: https://arxiv.org/pdf/1211.7081.pdf
[\[27\]]: https://ru.wikipedia.org/wiki/%D0%9A%D0%B2%D0%B0%D0%BD%D1%82%D0%BE%D0%B2%D0%BE%D0%B5_%D1%81%D0%B0%D0%BC%D0%BE%D1%83%D0%B1%D0%B8%D0%B9%D1%81%D1%82%D0%B2%D0%BE
[\[28\]]: https://en.wikipedia.org/wiki/The_Fabric_of_Reality
[\[29\]]: http://dmishin.blogspot.com/2014/06/cellular-automata-with-2-temporal.html
[\[30\]]: https://conwaylife.com/
[\[31\]]: https://conwaylife.com/wiki/Turing_machine
[\[32\]]: https://oimo.io/works/life/
[\[33\]]: https://mathstodon.xyz/@timhutton
[\[34\]]: https://github.com/timhutton/linear-enzymes
[\[35\]]: https://xkcd.ru/505/
[\[36\]]: https://t.me/def_model_train/927
[\[37\]]: https://www.lesswrong.com/posts/fZJRxYLtNNzpbWZAA/the-mathematical-universe-the-map-that-is-the-territory
[\[38\]]: https://zachaysan.tumblr.com/post/7238016119/statistical-immortality
[\[39\]]: https://www.gregegan.net/PERMUTATION/FAQ/FAQ.html
[\[40\]]: https://news.ycombinator.com/item?id=2727750
[\[41\]]: https://news.ycombinator.com/item?id=24385433


* [\[1\]] Mathematical universe hypothesis — Wikipedia
* [\[2\]] Permutation City — Wikipedia
* [\[3\]] Матрица (фильм) — Википедия
* [\[4\]] The Sims — Википедия
* [\[5\]] Тринадцатый этаж — Кинопоиск
* [\[6\]] Виртуальная реальность — Википедия
* [\[7\]] Игра «Жизнь» — Википедия
* [\[8\]] Creative — Minecraft Wiki
* [\[9\]] Finally We May Have a Path to the Fundamental Theory of Physics… and It’s Beautiful — Stephen Wolfram Writings
* [\[10\]] A Cognitive Computation Fallacy? Cognition, Computations
* [\[11\]] Проблема остановки — Википедия
* [\[12\]] Proof That Computers Can't Do Everything (The Halting Problem) — YouTube
* [\[13\]] Наша математическая вселенная — Википедия
* [\[14\]] Этернализм — Википедия
* [\[15\]] Causal Universes — LessWrong
* [\[16\]] Universality in Elementary Cellular Automata — Matthew Cook
* [\[17\]] Метод конечных элементов — Википедия
* [\[18\]] Бесконечная вложенность материи — Википедия
* [\[19\]] Computable number — Wikipedia
* [\[20\]] A definable number which cannot be approximated algorithmically
* [\[21\]] Гипотеза Коллатца — Википедия
* [\[22\]] Does a Rock Implement Every Finite-State Automaton? — David J. Chalmers
* [\[23\]] ДЖОН Р. СИРЛ Разум, мозг и программы
* [\[24\]] Measure problem (cosmology) — Wikipedia
* [\[25\]] Science Feature: Dust Theory — ScienceFiction.com
* [\[26\]] The Universe is not a Computer
* [\[27\]] Квантовое самоубийство — Википедия
* [\[28\]] The Fabric of Reality — Wikipedia
* [\[29\]] Cellular automata with 2 temporal dimensions - Shintyakov Dmitry
* [\[30\]] Conway's Game of Life online simulator
* [\[31\]] Turing Machine - LifeWiki
* [\[32\]] Recursive Game of Life simulation using OTCA metapixel
* [\[33\]] Tim Hutton - Mathstodon
* [\[34\]] GitHub - timhutton/linear-enzymes: Artificial chemistry where chains of atoms can catalyse reactions
* [\[35\]] Кучка камней - xkcd
* [\[36\]] Пост #927 в Telegram канале «я обучала одну модель»
* [\[37\]] The mathematical universe: the map that is the territory
* [\[38\]] Statistical immportality
* [\[39\]] Dust Theory FAQ — Greg Egan
* [\[40\]] Statistical Immortality — Hacker News
* [\[41\]] Statistical Immortality, another discussion — Hacker News
