...
{{ admonition_start(color="blue", title="📝 Вводимое определение") }}
**Иллюзорная симуляция** — такая симуляция, которая создаёт иллюзию реальности окружающего мира, путём обмана наблюдателя. Другое название таких симуляций — виртуальная реальность [\[3\]]. Пример таких симуляций: фильм «Матрица» [\[4\]], игра «The Sims» [\[5\]] (мир иллюзорен для симов), фильм «13-й этаж» [\[6\]].
{{ admonition_end() }}
...
Можно самому поиграться в более продвинутых симуляторах, например `conwaylife.com` [\[10\]].
...
Или вот пример искусственной химии, которую разработал Tim Hutton [\[13\]], в которой он спроектировал такие элементы, и такие взаиомодействия между ними, чтобы собрать искусственную клетку, способную размножаться:
...
{{ admonition_start(color="blue2", title="📖 Определение") }}
**Этернализм** — точка зрения на время, согласно которой будущие события уже существует, объективного «течения» времени не существует, и всё пространство-время может быть представлено как статичный неизменяемый «блок». [\[19\]]
{{ admonition_end() }}
...
* или если некоторое бессмертное существо производит это вычисление на клеточном автомате 110, перекладывая камешки, согласно простейшим правилам (автомат 110 является Тюьринг-полным [\[22\]]).
...
Если бы мы умели решать проблему останова, то это можно было бы использовать для доказательства некоторого вида теорем. Например, можно доказать или опровергнуть Гипотезу Коллатца (проблема 3x+1) [\[28\]]: пишем программу, которая перебирает все числа, и смотрит к чему сведётся данное число, если оно сведётся не к 1, то остановить программу. Если мы решим проблему останова для такой программы, то мы однозначно скажем, верна эта гипотеза или нет. Поэтому иметь машину по решению проблемы останова было бы невероятно удобно для исследования математики.
...
У Тегмарка [\[18\]] вся теория строилась на том, что из существования внешней физической реальности следует что она существует как математическая конструкция, но это ненмого рушится тем, что среди математических конструкций существуют и отдельные сознания, не обязательно привязанные к какой-то вселенной.
...
Так что из такого видения можно сказать, что ИИ на основе сухих алгоритмов перекладывания букв (как в мысленном эксперименте китайской комнаты Сёрла [\[31\]]) будет чувствовать мир совершенно не так как человек, но ИИ на основе нейросетей вполне может, а может даже лучше и прекрасней, как сверх-синестет.
...
В [\[18\]] Тегмарк утверждает что возможность умереть всё же есть: нужно чтобы ваше сознание постепенно затухало (как при болезни Альцгеймера), тогда если такое сознание окажется в другой вселенной после «смерти», то его носитель не будет ничего чувствовать, или чувствовать как при жизни.
